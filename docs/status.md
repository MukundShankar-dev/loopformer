...
| Stage 1 stepwise training | **First CUDA overfit run complete; Gate 1 unestablished** | [Training guide](training_pointer.md); intermediate loss, compact logging, per-loop validation, adapter checkpoints and resume, toy-model checks |
...
Stage 0 proves architecture correctness on small probes, not useful recurrent computation or one-loop/one-transition behavior. Long-unroll training memory, mixed precision, and MPS margin-loss backward remain unverified or unimplemented. Adapter persistence and exact CPU resume were verified with toy models on the original environment; the new WSL environment has the exact-resume precision failure recorded above. The first small pretrained run and its resource use are recorded in the overfit report; larger-scale resource use remains unverified; no future phase is authorized by this status entry.
