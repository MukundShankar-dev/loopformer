...
Batching update: the user stopped and deleted the first fresh run, then launched fresh adapters with batch 2/accumulation 4. Early desktop readings are approximately 5,623/16,303 MiB GPU usage in training, 3,233 MiB during validation, and an ETA near one hour. These are user-reported observations; no completed new training result is available. A schema-validated batch-4/accumulation-2 config is now prepared with the same eight examples per update and all other settings unchanged. Its GPU fit and throughput remain unverified; use explicit batch-change resume from a retained batch-2 checkpoint. See [batching instructions](training_pointer.md#resume-with-larger-microbatches).

The fresh 30k run is now running on the desktop after the startup fix. The user requested batching for throughput; their reported 3.7 GiB process peak is host RAM, not VRAM. A batch-2/accumulation-4 config and explicit same-effective-batch resume support are implemented, with optimizer/RNG/cursor preservation and recorded numerical-continuation caveats. The dashboard now labels host memory and CUDA tensor allocation separately. Thirteen focused training, startup, and depth-comparison tests pass in 34.99 seconds, including rejected identity changes, optimizer/cursor preservation, and a batched CLI resume. Config, CLI help, local documentation links, and whitespace checks pass. Desktop batched throughput and memory remain unmeasured. See [resume and tmux commands](training_pointer.md#resume-with-larger-microbatches).

Fresh-run startup: the desktop reported a T=1 equivalence failure for one vocabulary logit, approximately `1.3e-5` absolute difference. The gate now aligns ordinary-Qwen's LM-head projection with the wrapper's answer-only projection, keeping `atol=rtol=1e-5`. This addresses a plausible shape-dependent rounding source; the desktop CUDA retry has not been verified. The focused startup, T=1, and pointer-training suites pass (23 tests in 28.36 seconds on the Mac). Training and inference mechanics are unchanged. See [startup troubleshooting](training_pointer.md#fresh-depth-6-run-with-30000-mappings).
