...
Stage 0 proves architecture correctness on small probes, not useful recurrent computation or one-loop/one-transition behavior. Long-unroll training memory, mixed precision, and MPS margin-loss backward remain unverified or unimplemented. Adapter persistence and exact CPU resume were verified with toy models on the original environment; the new WSL environment has the exact-resume precision failure recorded above. Actual pretrained training and training-device resource use remain unverified. Actual Stage 1 resource use must be measured before scaling; no future phase is authorized by this status entry.
